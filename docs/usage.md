...
```bash
nextflow run fgualdr/nf_chipmm --input samplesheet.csv --outdir <OUTDIR> --genome GRCh37 -profile docker
```
...
```bash
nextflow pull fgualdr/nf_chipmm
```
...
First, go to the [fgualdr/nf_chipmm releases page](https://github.com/fgualdr/nf_chipmm/releases) and find the latest version number - numeric only (eg. `1.2.2`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.2.2`.
