...
```console
nextflow run fgualdr/nf_chipmm -profile test_full,<docker/singularity/institute>
```
...
> **NB:** Currently, paired-end files produced by `Chromap` are excluded from downstream analysis due to [this](https://github.com/fgualdr/nf_chipmm/issues/291) issue. Single-end files are processed normally.
